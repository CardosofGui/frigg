...
actual class FriggConverter actual constructor() {
    actual suspend fun convertWavToMp3(wavPath: String, bitrate: Int): Result<String> {
        ...
        if (!fileManager.fileExistsAtPath(wavPath)) {
            return Result.failure(Exception("Arquivo WAV não encontrado: $wavPath"))
        }
        
        if (!fileManager.isReadableFileAtPath(wavPath)) {
            return Result.failure(Exception("Sem permissão para ler o arquivo WAV: $wavPath"))
        }
        ...
        if (mp3Dir.isNotEmpty() && !fileManager.fileExistsAtPath(mp3Dir)) {
            if (!fileManager.createDirectoryAtPath(mp3Dir, true, null, null)) {
                return Result.failure(Exception("Não foi possível criar o diretório de saída: $mp3Dir"))
            }
        }
        
        if (mp3Dir.isNotEmpty() && !fileManager.isWritableFileAtPath(mp3Dir)) {
            return Result.failure(Exception("Sem permissão para escrever no diretório: $mp3Dir"))
        }
            ...
            if (result != 0) {
                if (fileManager.fileExistsAtPath(mp3Path)) {
                    ...
                    if (fileSize > 0) {
                        ...
                        return Result.failure(Exception("Conversão retornou sucesso, mas o arquivo MP3 está vazio: $mp3Path"))
                    }
                } else {
                    return Result.failure(Exception("Conversão retornou sucesso, mas o arquivo MP3 não foi criado: $mp3Path"))
                }
            } else {
                return Result.failure(Exception("A conversão falhou. Verifique se o arquivo WAV é válido e se há espaço em disco suficiente."))
            }
        } catch (e: Exception) {
            return Result.failure(Exception("Erro durante a conversão: ${e.message}", e))
        }
    }
}
